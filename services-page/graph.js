...
var db = new Image(); 
db.src = "/atlas-management/resources/kibana/app/panels/graph/imgs/database.png";
var cpu = new Image(); 
cpu.src = "/atlas-management/resources/kibana/app/panels/graph/imgs/computer.png";
var srv = new Image(); 
srv.src = "/atlas-management/resources/kibana/app/panels/graph/imgs/service.png";
	...
	var that = {
		...
		update: function( host ){
			...
			if( !sys )
				return;		
			var server = host;
			var endpoints = top.bn.performance.endpoints;
			var metrics = top.bn.performance.metrics;
			...
			sys.eachNode(function(node, pt){
				...
				for( var i in endpoints[node.name].metrics ){
					...
					if( avg >= top.bn.performance.getThreshold( metrics[mt].category ) ){
						health = "red";
						break;
					} // end if			
				} // end for
...
function getCurrentAverages( servers, callback ){
	...
	var metrics = top.bn.performance.metrics;
	var esurl = top.bn.performance.dashboard.esuri + "/logstash-atlas-performancemonitor/_search";
	var minutesago = top.bn.performance.metricavgforlastminutes;
	var d = new Date();
	var now = d.getTime(); 
...
function createNodesEdges(){
	var server   = top.bn.performance.dashboard.hosts;	// get the selected server(s)
	var endpoints = top.bn.performance.endpoints;		// get the endpoints
